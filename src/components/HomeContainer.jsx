...
export const HomeContainer = () => {
  return (
    ...
    >
      <div className="py-2 gap-6 flex flex-col flex-1 items-start  justify-start">
      ...
      <div className="py-2 relative  flex-1 items-center">
        <img
          ...
          alt="Hero Background"
        />
        <div className="w-full  h-full absolute top-0  left-0  flex items-center justify-center lg:px-32 py-4 gap-10 lg:gap-3 flex-wrap">
          {staticImg &&
            staticImg.map((item) => (
              <motion.div
                key={item.id}
                className="  lg:w-190  p-2 flex-col bg-cardOverlay backdrop-blur-md rounded-3xl flex items-center justify-center"
              >
                <img
                  src={item.source}
                  className="cursor-pointer w-20 lg:w-40 -mt-10  lg-mt-20"
                  alt="dish"
                />
                <p className="text-base lg:text-xl text-center font-semibold text-orange-700 mt-2 lg:mt-4 ">
                  {item.name}
                </p>
                <p className="text-[10px] lg:text-sm my-1 lg:my-3 text-orange-900 font-semibold text-center">
                  {item.desc}
                </p>
                <p>
                  <motion.button
